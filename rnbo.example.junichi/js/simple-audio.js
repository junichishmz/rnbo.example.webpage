...
async function setup() {
  ...
  //Create RNBO Device
  try {
    effectOne = await RNBO.createDevice({ context, patcher });
    effectTwo = await RNBO.createDevice({ context, patcher });
 ...
 // Now make a web audio node for the radio
  ...
  const inputNode2 = new MediaElementAudioSourceNode(context, {
    mediaElement: document.getElementById("bgm"),
    channelCount: 1
  });
  ...
  // Connect the device to the web audio graph
  inputNode.connect(effectOne.node);
  inputNode2.connect(effectTwo.node);

  effectTwo.node.connect(outputNode);
  effectOne.node.connect(outputNode);
  ...
  makeSliders(effectOne);
  makeSliders(effectTwo);
